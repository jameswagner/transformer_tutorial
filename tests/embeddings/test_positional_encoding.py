...
def test_shape(batch_size, sequence_length, d_model):
    ...
    pe = create_positional_encoding(sequence_length, d_model, batch_size)
    assert pe.shape == (batch_size, sequence_length, d_model)

def test_dtype_and_device(sequence_length, d_model):
    """Test tensor properties"""
    pe = create_positional_encoding(sequence_length, d_model, batch_size=1)
...
def test_deterministic(sequence_length, d_model, batch_size):
    ...
    pe1 = create_positional_encoding(sequence_length, d_model, batch_size)
    pe2 = create_positional_encoding(sequence_length, d_model, batch_size)
...
def test_even_odd_pattern():
    ...
    pe = create_positional_encoding(sequence_length=5, d_model=8, batch_size=1)
